...
struct segment_tree{
    ...
    : from(from), to(to), left(NULL), right(NULL), value(INF) { }
};
...
// range sum query
int min_query(segment_tree *tree, int l, int r){
    if (tree == NULL) return INF;
    if (l <= tree->from && tree->to <= r) return tree->value;
    if (tree->to < l) return INF;
    if (r < tree->from) return INF;
    return min(min_query(tree->left, l, r), min_query(tree->right, l, r));
}
...
int main() {
    ...
    cout << "range sum min before update: " << minn << endl;
    ...
    cout << "range min query after update: " << minn << endl;
    return 0;
}
